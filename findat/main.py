...
def process_directory(directory_path: str, key_string: str) -> dict[str, int]:
    ...
    for file_name in os.listdir(directory_path):
        if file_name.endswith(".txt"):
            ...
            resolved_text: str = coref_resolution(file_path)
            occurrences: int = resolved_text.lower().count(key_string.lower())
            file_occurrences[file_name] = occurrences
    return file_occurrences


def sort_files_by_occurrences(directory_path: str, key_string: str) -> tuple[str, int]:
    '''
    ...
    return sorted(
        file_occurrences.items(), key=lambda x: x[1], reverse=True
    )
...
def main() -> None:
    ...
    sorted_files: tuple[str, int] = sort_files_by_occurrences(directory_path, key_string)
    print(
